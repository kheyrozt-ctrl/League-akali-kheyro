...
export class OpggDataApi {
  ...
  async getChampionSynergies(options: {
    id: number
    region: RegionType
    tier: TierType
    position: PositionType
    version?: string
    signal?: AbortSignal
  }): Promise<OpggChampionSynergies> {
    const { id, region, tier, position, version, signal: abortSignal } = options

    const data = await this._http.get(
      `/api/${region}/champions/ranked/${id}/${position}/synergies`,
      {
        params: { tier, version },
        signal: abortSignal
      }
